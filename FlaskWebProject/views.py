...
def authorized():
    ...
    if "error" in request.args:  # Authentication/Authorization failure
        app.logger.warning("Microsoft login error: %s", request.args.get("error_description"))
        return render_template("auth_error.html", result=request.args)
    if request.args.get('code'):
        cache = _load_cache()
        # Acquire a token from MSAL app
        ...
        if "error" in result:
            app.logger.warning("Microsoft login failed: %s", result.get("error_description"))
            return render_template("auth_error.html", result=result)
        session["user"] = result.get("id_token_claims")
        app.logger.info("Microsoft user %s logged in successfully", session["user"].get("name"))
        # For this project, map MS user to admin account
